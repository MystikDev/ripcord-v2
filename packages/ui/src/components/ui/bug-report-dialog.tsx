...
export function BugReportButton({ inline }: { inline?: boolean } = {}) {
  ...
  return (
    <>
      <button
        ...
        className={
          inline
            ? 'flex h-8 w-8 items-center justify-center rounded-full shrink-0 border border-border bg-white/3 hover:bg-accent/20 hover:border-accent/40 transition-colors cursor-pointer'
            : 'fixed bottom-16 right-4 z-40 flex h-10 w-10 items-center justify-center rounded-full bg-surface-2/80 border border-border backdrop-blur-sm shadow-lg hover:bg-accent/20 hover:border-accent/40 transition-colors'
        }
        title="Report a bug"
      >
        <svg width={inline ? '14' : '18'} height={inline ? '14' : '18'} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-text-muted">
