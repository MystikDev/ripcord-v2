/**
 * @module floating-friends-panel
 * Floating, draggable, resizable, pinnable Friends panel for the orbital view.
 * Follows the same architecture as FloatingChatPanel — glass-morphism window
 * with title-bar drag handle, pin/close controls, and edge/corner resize handles.
 *
 * Content: embeds the existing <FriendsPanel /> component directly.
 */
'use client';
...
interface FloatingFriendsPanelProps {
...
function getDefaultPosition(width: number, height: number) {
  return {
    x: Math.max(16, window.innerWidth - width - 140),
    y: Math.max(16, window.innerHeight - height - 70),
  };
}
